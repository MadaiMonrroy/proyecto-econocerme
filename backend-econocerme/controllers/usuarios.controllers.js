...
export const listaUsuarios = async (req, res) => {
  try {
    const [result] =
      await connection.query(`SELECT id, nombres, primerApellido, segundoApellido, email, IFNULL(fotoPerfil, '${process.env.BASE_URL}/uploads/usuarios/avatar3.png') AS fotoPerfil, 
 fechaNacimiento, tipoUsuario, estado FROM usuario WHERE estado = 1 OR estado = 2`);
    res.json(result);
  } catch (error) {
    console.error(error);
    res.status(500).json({ mensaje: "Ocurrió un error en el servidor" });
