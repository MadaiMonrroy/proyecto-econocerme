  ...
  <div class="card min-h-screen flex flex-col p-6 lg:p-12 bg-gray-100">
    ...
    <div class="mb-8">
      ...
      <p class="text-sm text-gray-500">
        Última actualización: {{ formatearFecha(leccion.ultimaActualizacion) }}
      </p>
    </div>
    ...
    <div
      class="card flex flex-col lg:flex-row justify-between items-center bg-white p-4 rounded-lg shadow-md"
    >
      <Button
        label="Lección Anterior"
        icon="pi pi-arrow-left"
        class="p-button-outlined mb-4 lg:mb-0"
      />
      <Button
    ...
    const cargarLeccion = async () => {
      try {
        const response = await api.get(
          `/lecciones/obtenerLeccion/${idLeccion.value}`
        );
