    ...
    <div class="card shadow-2xl dark:shadow-violet-600">
      ...
      >
        ...
        >
          ...
          <Column header="Curso" class="px-6 py-4">
            <template #body="rowData">
              <div class="flex gap-4">
                <div class="flex items-center justify-center">
                  <Image
                    ...
                    alt="Foto de Perfil"
                    class="object-cover object-center h-16 w-16"
                    preview
                  />
                </div>

                <div class="flex flex-col justify-center items-start">
                  <p>{{ rowData.data.titulo }}</p>
                  <Tag severity="info" :value="rowData.data.especialidad"></Tag>
                </div>
              </div>
            </template>
          </Column>
...
const fetchData = async () => {
  try {
    const response = await api.get(
      `/inscripciones/listaInscritosCoach/${idUsuario}`
    );
    usuarios.value = response.data.reverse();

    // Obtener títulos únicos y ordenados
    const titulos = usuarios.value
      .map((item) => item.titulo)
      .filter((titulo) => titulo) // Eliminar valores null/undefined
      .filter((titulo, index, self) => self.indexOf(titulo) === index) // Eliminar duplicados
      .sort(); // Ordenar alfabéticamente

    titulosData.value = titulos.map((titulo) => ({
      label: titulo,
      value: titulo,
    }));
  } catch (error) {
...
const usuariosConNumeracion = computed(() => {
  ...
  let filteredUsers = usuarios.value;

  // Primero aplicamos el filtro por título si hay uno seleccionado
  if (selectedTitulo.value) {
    filteredUsers = filteredUsers.filter(
      (usuario) => usuario.titulo === selectedTitulo.value
    );
  }

  // Luego aplicamos el filtro global si existe
  if (filter) {
    filteredUsers = filteredUsers.filter((usuario) => {
      ...
      const titulo = usuario.titulo?.toLowerCase() || "";
  ...
  // Finalmente, agregamos el índice a los usuarios filtrados
  return filteredUsers.map((usuario, index) => ({
    ...usuario,
    index: index + 1,
    expanded: false,
  }));
