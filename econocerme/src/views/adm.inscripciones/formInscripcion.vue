<template>
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <MultiSelect v-model="inscripcion.cursoId" :options="cursos" optionLabel="titulo" filter
          ...
          <div>
            <label for="cantidadCuotas" class="block text-sm font-medium mb-2">Elige la Cantidad de Cuotas</label>
            <div class="card grid grid-cols-[1fr_4px_1fr] gap-4 items-center p-4">
              <div class="flex flex-col space-y-2 items-center">
                <label for="cursoId" class="block text-sm font-medium mb-2">Nro. Cuotas</label>
                <SelectButton v-model="inscripcion.cantidadCuotas" :options="cuotas" allowEmpty :invalid="inscripcion.cantidadCuotas === null" />
              </div>

              <!-- Divider -->
              <Divider layout="vertical" class="h-72 border-x-2 border-green-400" />

              <!-- Montos de cuotas -->
              <div class="ml-4 flex flex-col space-y-2">
                <div v-for="(cuota, index) in cuotasComponentes" :key="index" class="flex items-center">
                  <Message :severity="cuota.pagado ? 'success' : 'warn'" icon="pi pi-money-bill">
                    Cuota {{ index + 1 }} - {{ cuota.pagado ? 'Pagado' : 'A Pagar' }}
                    <Tag :value="cuota.pagado ? 'Pagado' : 'Pendiente'" :severity="cuota.pagado ? 'success' : 'warning'"></Tag>
                    <br />
                    <Tag :value="`Fecha de Pago: ${cuota.fecha}`" :severity="cuota.pagado ? 'success' : 'info'"></Tag>
                    <br />
  ...
  <Dialog v-model:visible="isConfirmModalOpen" header="Confirmar Primera Cuota Pagada" modal class="w-full max-w-sm">
    <div class="p-4">
      <p>La Inscripcion del Estudiante necesita confirmar que la primera cuota ha sido pagada.</p>
      <form @submit.prevent="agregarAFormInscripcion">
        <div class="grid grid-cols-1 gap-4 p-4">
          <div>
            <label for="montoCuota" class="block text-sm font-medium  mb-2">Monto de la Primera Cuota</label>
            <InputText v-model="selectedCuota.montoCuota" type="number" id="montoCuota"
              class="block w-full text-sm border-gray-300 rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              required step="0.01" :readonly="true" />
          </div>
          <div>
            <label for="metodoPago" class="block text-sm font-medium  mb-2">Método de Pago</label>
            <Select v-model="selectedCuota.metodoPago" :options="metodosPago" optionLabel="label" optionValue="value"
              placeholder="Seleccione un Metodo de Pago" checkmark :highlightOnSelect="false"
        ...
        <div class="p-4 flex justify-end space-x-4">
          <Button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-md"
            severity="success">Confirmar</Button>
          <Button type="button" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-md" severity="secondary"
...
watch(() => inscripcion.cantidadCuotas, (nuevaCantidad) => {
  console.log("CAMBIOO")
...
watchEffect(() => {
  ...
  if (inscripcion.cantidadCuotas >= 2) {
    ...
    const montoConRecargo = calcularMontoTotal.value + (calcularMontoTotal.value * 0.05);
...
watch(() => montoTotal.value, (nuevoMontoTotal) => {
  if (inscripcion.cantidadCuotas) {
    const montos = calcularMontoCuotas(inscripcion.cantidadCuotas, nuevoMontoTotal);
...
const agregarAFormInscripcion = async () => {
  ...
  try {
    ...
    const response = await api.post("/inscripciones/agregarInscripcion", payload);
    ...
    if (response) {
      toast.add({
        ...
        detail: "La inscripción y el primer pago han sido registrados correctamente.",
        life: 3000,
      });
