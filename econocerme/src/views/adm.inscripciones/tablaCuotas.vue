...
<template>
    <div class="p-4">
      <div class="card">
        ...
        <Divider />
        <div class="card">
          <DataTable :value="cuotas" :tableStyle="{ 'min-width': '50rem' }" :rows="4" :paginator="true"
            :rowsPerPageOptions="[4, 8, 12]" class="p-datatable-striped" scrollable scrollHeight="61vh" stripedRows lazy>
            ...
            <Column field="metodoPago" header="Metodo de Pago" sortable class="px-6 py-4" />
            <Column field="estadoCuota" header="Estado" class="px-6 py-4">
              <template #body="rowData">
                <Tag v-if="rowData.data.estadoCuota === 0" value="Eliminado" severity="danger" class="px-2 py-1" />
                <Tag v-else-if="rowData.data.estadoCuota === 1" value="Pagado" severity="success" class="px-2 py-1" />
                <Tag v-else-if="rowData.data.estadoCuota === 2" value="Pendiente" severity="warn" class="px-2 py-1" />
                <Tag v-else value="Desconocido" severity="warning" class="px-2 py-1" />
              </template>
            </Column>
            <Column field="fecha" header="Fecha de Vencimiento" sortable class="px-6 py-4">
              <template #body="rowData">
            ...
            <Column header="Acciones" class="px-6 py-4">
              <template #body="rowData">
                <div class="flex items-center space-x-2">
                  ...
                  <Button icon="pi pi-money-bill" class="p-button-rounded p-button-info"
      ...
      <Dialog v-model:visible="isConfirmModalOpen" header="Confirmar Eliminación" modal class="w-full max-w-sm">
        <div class="p-4">
          ...
          <div class="mt-4 flex justify-end space-x-4">
            <Button class="bg-red-500 text-white px-4 py-2 rounded-md" @click="deleteUsuario">Eliminar</Button>
      ...
      <Dialog header="Detalles de la Cuota" v-model:visible="showUserDetailsModal" modal :style="{ width: '50vw' }"
      ...
      <Dialog v-model:visible="isModalOpen" :header="isEditMode ? 'Editar Cuota' : 'Agregar Cuota'" :visible="isModalOpen"
        modal class=" " :style="{ zIndex: 10 }">
        <form @submit.prevent="isEditMode ? updateCuota() : addCuota()">
          <div class="grid grid-cols-2 gap-4 p-4">
            <div>
              <label for="montoCuota" class="block text-sm font-medium  mb-2">Monto de la Cuota</label>
            ...
            <div class="field">
              ...
              <Select v-model="selectedCuota.estadoCuota" :options="estadosCuota" optionLabel="label" optionValue="value"
                placeholder="Seleccione el Estado Actual de la Cuota" checkmark :highlightOnSelect="false"
                class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" />
            </div>
            <div class="field">
  ...
  const openEditModal = (cuota) => {
    ...
    selectedCuota.value.fechaVencimiento = formatToDateInput(cuota.fechaVencimiento);
  ...
  const getRowIndex = (rowData) => {
    return cuotas.value.findIndex((cuota) => cuota.idCuotaPago === rowData.data.idCuotaPago) + 1;
  };
