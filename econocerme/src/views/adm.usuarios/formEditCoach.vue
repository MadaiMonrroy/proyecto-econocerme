...
const agregarEspecialidad = () => {
  if (especialidadInput.value.trim() === "") return; // Evitar agregar vacíos
  especialidades.value.push(especialidadInput.value.trim());
  especialidadInput.value = ""; // Limpiar el input
};
...
const obtenerUsuario = async (idUsuario) => {
  try {
    ...
    if (Array.isArray(selectedUsuario.value.especialidad)) {
      especialidades.value = selectedUsuario.value.especialidad; // Asegúrate de que esto sea un array
    } else {
      especialidades.value = []; // Inicializar como vacío si no hay especialidades
    }
...
const updateUsuario = async () => {
  ...
  formData.append("biografia", selectedUsuario.value.biografia);
  formData.append("experiencia", selectedUsuario.value.experiencia);
  formData.append("especialidad", JSON.stringify(especialidades.value)); // Convertir a JSON
